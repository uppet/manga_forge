...
func _process(delta: float) -> void:
	if boot_smoke_frame >= 0:
		...
		if boot_smoke_frame >= 120:
			var analytics_info := {"available": false, "status": "unavailable", "embedded": false, "credential_profile": "none", "config_fingerprint": "none"}
			var analytics := get_node_or_null("/root/GameAnalyticsClient")
			if analytics != null:
				analytics_info = analytics.build_credential_info()
			print("INKBOUND_EXPORTED_BOOT_OK version=%s renderer=%s frames=%d analytics_embedded=%s analytics_profile=%s analytics_config=%s analytics_available=%s analytics_status=%s" % [
				...
				str(bool(analytics_info.get("available", false))).to_lower(),
				str(analytics_info.get("status", "unknown")),
			])
			boot_smoke_frame = -1
			get_tree().quit(0)
		return
